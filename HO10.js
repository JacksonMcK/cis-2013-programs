...
var passwordCheckOld = function () 
{
    ...
    stringOutput = ""; //Sets the stringOutput variable to empty to begin the program
 
    if (stringPassword.length < 8) {
      //Checks to see if the user-entered password is less than 8 characters long
      stringOutput = "Password is not long enough.";
      //Setting the stringOutput to tell the user that their password is not long enough
      $("output").value = stringOutput;
       } else {
         //If the password is 8 characters or longer it is put through a for loop
            for (i = 0; i < arrayBadPasswords.length; i++) {
               //Parses through the Array
               if (stringPassword === arrayBadPasswords[i]) {
                  //Checks the Password against the array of bad passwords
                  stringOutput = "The password that you have entered has been deemed bad. Please try again.";
                  $("output").value = stringOutput;
                  break;
            }  else {stringOutput = "Your password is good!";
            //Entered if the password is 8 or longer characters and is not found in the array of bad passwords
                   $("output").value = stringOutput;
            }       
         }
      }
